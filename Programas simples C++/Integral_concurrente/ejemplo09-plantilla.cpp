...
double funcion_hebra( long i )
{
   	...
   	for(long j=((m/n)*i); j<((m/n)*(i+1)); j++){ //long j=i; j<m; j+=n
		const double xj=double(j+0.5)/m;     
		suma+=f(xj);                        
	}
   	
	return suma; 
  //Hacer calculos de las hebras teniendo en cuenta el indice de la hebra
}
...
double calcular_integral_concurrente( )
{
	...
	for(int i=0; i<n; i++){	//NO meter esto en el for de arriba, si se hace no es paralelo por que el get espera a que acabe la hebra
		solucion+=futuros[i].get();		
	}
	return solucion/m;

  // lanza n hebras con async y crea un vector de future
}
// -----------------------------------------------------------------------------
