...
class recetas{
   private:
    ...
    public:
      ...
      void borrar(const string & i);

      map<string, receta> getDatos();

      const map<string, receta> getDatos()const;

      void calcularValores(ingredientes I);

      receta fusionar(const string & cod1, const string & cod2);
     ...
     receta operator[](const string &cod)const{
      ...
      class const_iterator{
		  ...
		  friend ostream & operator<<(ostream &os,const recetas &R);
    	  friend istream & operator>>(istream &os,recetas &R);
